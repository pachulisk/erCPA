...
;;; Result facts are retracted after processing to prevent re-firing
;;; ============================================================

(defrule mark-success
  "Clear cooldown on successful response"
  (declare (salience 100))
  ?r <- (result (credential-id ?cid) (model ?m) (status-code ?s))
...
(defrule mark-rate-limited
  ...
  (declare (salience 100))
  ?r <- (result (credential-id ?cid) (model ?m) (status-code 429) (now ?now))
...
(defrule mark-unauthorized
  ...
  (declare (salience 100))
  ?r <- (result (credential-id ?cid) (model ?m) (status-code ?s) (now ?now))
