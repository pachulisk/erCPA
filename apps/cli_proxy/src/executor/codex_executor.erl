-module(codex_executor).
-behaviour(gen_server).

%% Codex (OpenAI) provider executor
%% Sends requests to https://api.openai.com/v1/responses or chat/completions

-export([start_link/1, execute/4, execute_stream/4]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(DEFAULT_BASE_URL, <<"https://api.openai.com">>).
...
build_url(Auth, Opts) ->
    ...
    Alt = maps:get(alt, Opts, undefined),
    case Alt of
        <<"responses/compact">> ->
            <<BaseURL/binary, "/v1/responses">>;
        _ ->
